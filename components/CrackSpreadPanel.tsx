/**
 * US 3-2-1 Crack Spread panel.
 *
 * The 3-2-1 crack is the cleanest single proxy for US refining margin: what a
 ...
 * have any incentive to keep running — and will product (pump/diesel) prices
 * actually follow crude down, or stay stubbornly high?
 *
...
export interface CrackData {
  ...
  components: { wtiUsd: number; gasolineUsdGal: number; ulsdUsdGal: number };
...
function Sparkline({ points }: { points: { date: string; crackUsd: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-14" preserveAspectRatio="none" role="img"
         aria-label={`3-2-1 crack spread trend, latest $${last.crackUsd.toFixed(2)} per barrel`}>
...
export default function CrackSpreadPanel({ data }: { data: CrackData }) {
  ...
  // red-for-up, green-for-down convention used on the crude benchmark tiles.
      ...
      <div className="px-5 py-4">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
          <div className="rounded border border-oil-800/60 bg-oil-950/40 py-2">
            <p className="text-[10px] text-gray-500 uppercase tracking-wide">WTI</p>
            <p className="text-sm font-mono font-semibold text-white">${c.wtiUsd.toFixed(2)}</p>
